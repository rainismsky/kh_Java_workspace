...
<table border="0" cellpadding="0" cellspacing="0">
	...
	<td>
		<c:if test="${count > 0}">
			<br>
			<article>
				Catecory ▶  ${l_category_name} ▶  ${s_category_name}</a>
			</article>
			<br>
		...
		<div style="text-align: center;">
			<c:if test="${count > 0}">
				...
				<c:if test="${startPage > 10}">
					<a href="/Treasure/E/list.do?pageNum=${startPage - 10 }&l_category_name=${l_category_name}&s_category_name=${s_category_name}">[이전]</a>
				</c:if>

				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<a href="/Treasure/E/list.do?pageNum=${i}&l_category_name=${l_category_name}&s_category_name=${s_category_name}">[${i}]</a>
				</c:forEach>

				<c:if test="${endPage < pageCount}">
					<a href="/Treasure/E/list.do?pageNum=${startPage + 10}&l_category_name=${l_category_name}&s_category_name=${s_category_name}">[다음]</a>
				</c:if>
			</c:if>
		</div>
	</td>
